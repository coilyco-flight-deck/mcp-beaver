...
* `server-info` (optional `name=`) - mints one read-only tool reporting the server's own identity, mode, and tool inventory. Opt-in precisely because deny-by-absence is the model, and it restores the liveness probe 2026-07-28 removed along with `ping`.
